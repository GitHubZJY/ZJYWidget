...
public class Camera2 extends CameraViewImpl {
    ...
    public boolean setAspectRatio(AspectRatio ratio) {
        ...
        if (!isCameraOpened()) {
            mAspectRatio = ratio;
        } else if (mPreviewSizes.ratios().contains(ratio)) {
            mAspectRatio = ratio;
        } else {
            return false;
        }
    ...
    private boolean chooseCameraIdByFacing() {
        try {
            ...
            mFacing = CameraConstants.FACING_BACK;
            return true;
        } catch (CameraAccessException e) {
            if (BuildConfig.DEBUG) {
                Log.e("Camera", "Failed to get a list of camera devices", e);
            }
            return false;
        }
    }
    ...
    private void collectCameraInfo() {
        ...
        if (!mPreviewSizes.ratios().contains(mAspectRatio)) {
            if (mPreviewSizes.ratios().contains(CameraConstants.DEFAULT_ASPECT_RATIO)) {
    ...
    private boolean startOpeningCamera() {
        try {
            ...
            if (BuildConfig.DEBUG) {
                Log.e("Camera", "Failed to open camera: " + mCameraId, e);
            }
            return false;
        }
        return true;
    ...
    void startCaptureSession() {
        ...
        try {
            ...
            if (BuildConfig.DEBUG) {
                Log.e("Camera", "Failed to start camera session.");
            }
        }
    }
    ...
    private void lockFocus() {
        if (mCaptureCallback != null && mCaptureSession != null && mPreviewRequestBuilder != null) {
            try {
    ...
    void captureStillPicture() {
        try {
            ...
            captureRequestBuilder.set(CaptureRequest.JPEG_ORIENTATION,
                    (sensorOrientation +
                            mDisplayOrientation * (mFacing == CameraConstants.FACING_FRONT ? 1 : -1) +
                            360) % 360);
